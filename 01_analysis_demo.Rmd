...
here::i_am("MM/code/01_analysis_demo.Rmd")
library(tidyverse)
library(here)
library(readxl)
library(glue)
...
```{r}
demo_data <- read_rds(here("MM/code/demo_data.rds"))
...
dm1 <- dm1 %>% 
  ...
  bind_cols(time_sd=results_mix[[1]]$data[[1]]$time_sd,
...
plot <- dm1 %>% 
  mutate(cacostat=factor(cacostat,levels=c(0,1),labels=c("control","case"))) %>% 
 ggplot(aes(x=time_sd, y=mean, color=cacostat))+
        geom_point(alpha=0.5) +
        scale_color_manual(values = c("#2d7bd1", "#ea801c"))+
  geom_segment(aes(x = -42, xend = -1, y = est.slope[1]+est.slope[6]*-42, yend = est.slope[1]+est.slope[6]*-1), color="#2d7bd1", linewidth=1.2 )+
        geom_segment(aes(x = -42, xend = -1, y = est.slope[2]+ est.slope[1] + est.slope[7]*-42, yend = est.slope[2]+ est.slope[1]+ est.slope[7]*-1), color="#ea801c", linewidth=1.2 )+
  ylim(-2,3)+
        labs(x="Years before diagnosis",
             y=marker,
             color = "Group")+
        theme_bw()
